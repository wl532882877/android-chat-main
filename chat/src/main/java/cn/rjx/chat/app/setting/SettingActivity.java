...
import cn.rjx.chat.app.main.SplashActivity;
...
import cn.rjx.chat.kit.settings.PrivacySettingActivity;
        ...
        switch (requestCode) {
            case REQUEST_IGNORE_BATTERY_CODE:
                if (resultCode == RESULT_CANCELED) {
                    Toast.makeText(this, "允许Talk Now后台运行，更能保证消息的实时性", Toast.LENGTH_SHORT).show();
                }
                break;
            default:
    ...
    void diagnose() {
        ...
        OKHttpHelper.get("http://" + Config.IM_SERVER_HOST + "/api/version", null, new SimpleCallback<String>() {
            ...
            public void onUiFailure(int code, String msg) {
                ...
                Toast.makeText(SettingActivity.this, "访问Talk Now Server失败", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                ...
                if (!pm.isIgnoringBatteryOptimizations(packageName)) {
                    ...
                    Toast.makeText(this, "已忽略电池优化，允许Talk Now后台运行，更能保证消息的实时性", Toast.LENGTH_SHORT).show();
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
